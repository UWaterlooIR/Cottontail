...
- [ ] #2 The indexer writes the flat dump at <burrow>/docid-cp.tsv (inside the burrow dir), docid<TAB>cp, one line per indexed row, cp matching that row :item start. (A docno-less corpus writes no flat file.)
