...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 S1 run captured: tool-call emission rate, args-parse rate, and compile rate over the 10 baseline topics, with failure classification
- [ ] #2 S2 pre-check documented (starred tokens through mt-compile AND cover_rewrite end-to-end), and the stem* prompt run captured with usage + compile results
- [ ] #3 S3 captured: >=4 general-web needs x 3 turns against the live server, with per-turn compile validity, reasoning sizes, and an adapt-vs-repeat assessment
- [ ] #4 FINDINGS.md summarizes all three with go/no-go verdicts and the prompt deltas; results reported to Mark
- [ ] #5 TASK-22 depends on this task and its references are refreshed (src/mt.cc -> gcl/mt.cc)
<!-- AC:END -->
