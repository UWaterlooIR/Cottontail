...
- ONE LLM call judges ONE document. The Judger runs up to `[agents.judger].concurrency` calls
  simultaneously via a `ThreadPoolExecutor` (DEFAULT 15, the dev-GPU throughput peak — see
  "Judger serving scout"); the draft prompt + schema are under "The prompts".
- REASONING-MODEL serving (gpt-oss-120b reasons before answering): guided decoding must
  constrain ONLY the post-thinking / `final`-channel output, **never the thinking trace**
  (constraining the whole completion strangles the decomposition). The four-step reasoning
  happens in the model's thinking; `reason` is only the surfaced one-to-three-sentence
  justification — do NOT add a scratchpad/analysis field. `reasoning_effort` DEFAULTS to **"high"**
  (gpt-oss-120b's top tier) — it is the dominant throughput/cost lever (decode is the bottleneck;
  see the scout), configurable, and forwarded via `extra_body`.
...
def page_and_judge(query, judged, recorded, events, intent_budget):
    ...
    buffer = []                 # ranked hits fetched but not yet processed
    exhausted = False
    while not exhausted and len(recorded) < intent_budget:
        if not buffer:                           # REFILL: fetch the NEXT batch
            try:
        ...
        # ONE WAVE in rank order, judge_concurrency wide -> the judges run IN PARALLEL.
        wave = buffer[:cfg.judge_concurrency]; buffer = buffer[cfg.judge_concurrency:]
        new = [h for h in wave if h.cp not in judged]            # only NEW docs go to the Judger
        docs = [(h.summary, engine.read(h.cp)) for h in new]     # full doc + summary, NOT cp
        verdicts = { h.cp: v for h, v in zip(new, judger.judge(intent, docs)) }  # Verdict{reason,grade}
        ...
        for h in wave:                            # SCAN IN RANK ORDER (judging finished out of order)
            ...
            else:                                 # NEW: judge + RECORD -- ALWAYS, never discarded (see below)
            ...
            if relevant(g):
                ...
                streak += 1
                if streak >= cfg.nonrelevant_streak and not exhausted:
        ...
        if cfg.max_list_depth and depth >= cfg.max_list_depth:
            break
    return summarize(query, depth, again, fresh)
```

`relevant(grade)` := `grade >= cfg.relevant_grade_threshold`; DEFAULT threshold `1`, so only a
grade-0 (Irrelevant) doc extends the streak and any grade >= 1 resets it (i.e. "non-relevant for
the streak" = grade 0). It is a knob, but no longer a `# TODO`. **RETAIN ALL JUDGED DOCS:** the
streak only decides WHEN TO STOP descending (no further waves/fetches) -- it NEVER discards a
verdict. Every NEW doc the Judger graded is recorded and reported, even the docs that sit past
the streak trip within the wave that tripped it (they were judged in parallel; throwing their
verdicts away would waste real work). Over-judging is therefore bounded to < one wave and is
fully kept. The streak runs over the TRUE list (new AND prior-judged docs, in rank order); a
prior-judged doc uses its STORED grade.
...
- **A single ranked list is exhausted** when `nonrelevant_streak` consecutive non-relevant
  docs (DEFAULT: grade 0 only) occur in RANK ORDER over the TRUE list (new + prior-judged), or
  the list goes dry (a page returns no results). The streak only STOPS further descent; the
  wave that trips it is recorded in full (retain-all). Descent is finite (bounded by the streak,
  dry, or `intent_budget`; optional `max_list_depth` safety cap).
...
- `[agents.judger]` — `class`, `llm` (a `[llm.*]` profile; may differ from the Searcher's —
  ...
  as a validate-first tier, not a drop-in swap), `concurrency` (DEFAULT **15** — the throughput
  peak on the dev GPU per the serving scout below; this is BOTH the in-flight cap AND the wave
...
- Loop knobs (controller): `nonrelevant_streak` (default 5), `max_judgments` (default 1000 —
  ...
  constraint, so this just bounds the worst-case prefill, not KV), `relevant_grade_threshold`
  (DEFAULT **1**: non-relevant-for-the-streak = grade 0; grade >= 1 resets it), and an optional
  `max_list_depth` (per-query descent safety cap; default None/off). The wave width is
  `[agents.judger].concurrency` (no separate `judge_concurrency` knob — they were unified).

## Judger serving scout — findings (TASK-16, dev GPU)

`isj/scouting/scout_judger.py` swept concurrency over real ClimbMix docs (from `runs/bear-e2e3`)
at `reasoning_effort=high`, `max_doc_chars=50000`, measuring client throughput/latency paired
with vLLM KV/queue metrics. Generation throughput by concurrency: **C=1 ~180, C=8 ~580, C=16
~900, C=20 ~770 tok/s** (peak ~16, regresses by 20). At every level `Running` reached the target
with `Waiting=0` and `GPU KV cache <= ~21%` (no preemptions, no OOM). CONCLUSIONS: the binding
constraint is **decode bandwidth, not KV** (KV is never near full at these doc sizes on this
GPU); past ~16 concurrent sequences aggregate throughput DROPS and latency climbs. We therefore
set `concurrency = 15` (a clean number just under the peak) and keep `max_doc_chars = 50000`
generous (it bounds prefill, not KV).
...
run-output directory layout; RRF/fusion (still dropped). (`relevant_grade_threshold` default is
now decided — `1`, non-relevant-for-the-streak = grade 0; still a configurable knob.)
<!-- SECTION:DESCRIPTION:END -->

## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Searcher keeps the EXISTING single `search{query}` tool (no new tool; the `judge` tool is REMOVED; tool use FORCED so it always searches) and has NO relevance scale (the 0-3 rubric lives only in the Judger); given an intent plus its running history it emits one precise GCL boolean query per turn
- [x] #2 Judger judges ONE document per LLM call from the cover-biased summary + FULL document text, returning a Verdict{reason, grade} guided-decoded to the canonical UMBRELA/TREC 0-3 scale — reason declared BEFORE grade, grade Literal[0,1,2,3], NO cp (the controller pairs the verdict with the cp it asked about); up to [agents.judger].concurrency calls run in parallel; deterministic under a stub client in tests
- [x] #3 judger.md is a decomposed UMBRELA-style 0-3 prompt: steps intent -> topical match -> trust -> scope -> grade, with low TRUST capping the grade (open-web ClimbMix text), a reason that cites a concrete span/detail, and NO persona; guided decoding constrains ONLY the post-thinking / final-channel output, never the thinking trace
- [x] #4 Judging input is the FULL document text (truncated to max_doc_chars, default 50000), not the cover summary; the cp is never sent to the Judger model
- [x] #5 Controller fetches a LARGE batch per query (top_k=fetch_k, default 200) in ONE cover_search request and descends that TRUE ranked list client-side in WAVES of [agents.judger].concurrency (default 15, judged in parallel); only if the streak is not reached by the end of the batch does it continue with exclude=seen (this query's already-received cps, NOT the global judged set) to pull the NEXT batch; prior-judged docs still appear at their ranks and are counted; it stops further descent on nonrelevant_streak in rank order or when the list goes dry (optional max_list_depth)
- [x] #6 RETAIN ALL JUDGED DOCS: every NEW document the Judger graded is recorded and reported — including docs past the streak trip within the wave that tripped it (judged in parallel; never discarded). The streak only decides WHEN TO STOP descending, never which verdicts to keep; over-judging is bounded to < one wave and fully kept
- [x] #7 The non-relevant streak is computed in rank order over the TRUE list — including prior-judged docs (via their STORED grade) — even though parallel judging completes out of order; a doc is non-relevant for the streak when grade < relevant_grade_threshold (DEFAULT 1, i.e. only grade 0 extends the streak; grade >= 1 resets it)
- [x] #8 A document judged in a prior query is never re-sent to the Judger, never re-read, and never re-recorded; on re-encounter it is only COUNTED (it drives the streak via its stored grade and rolls into the query's already-judged aggregate)
- [x] #9 A malformed query (EngineError) or a zero-result query is fed back to the Searcher immediately as the search tool result, and the Searcher reformulates within the same conversation
- [x] #10 What the Searcher sees back is the NEW docs only (cover-biased SUMMARIES + grade + reason; NEVER full text), PLUS an aggregate for the prior-judged docs at those ranks: judged to depth K, of which J were already judged (X relevant, Y non-relevant) and are NOT relisted
- [x] #11 An intent stops ONLY when its recorded judgments reach its per-intent intent_budget or the max_queries backstop (default 100) trips; there is no Searcher-decline/no-tool-call stop (the search tool is forced)
- [x] #12 max_judgments (default 1000) is the RUN-TOTAL judgment budget; the Orchestrator splits it evenly into intent_budget = max_judgments // num_intents (>=1) and passes it to each controller (1000 over 2 intents = 500 each); budget unused by an intent that ends early is NOT reallocated
- [x] #13 Config adds [agents.judger] (class; llm — recommend a strong judge, default gpt-oss-120b, Gemma 4 edge as validate-first; concurrency DEFAULT 15 = in-flight cap AND wave width; reasoning_effort DEFAULT 'high') and loop knobs fetch_k (default 200), window, max_queries (default 100), nonrelevant_streak (default 5), max_judgments (RUN-TOTAL, default 1000), max_doc_chars (default 50000), relevant_grade_threshold (DEFAULT 1 = non-relevant streak is grade 0; configurable, no longer TODO), and optional max_list_depth
- [x] #14 Controller returns the existing SearcherResult{ranked_list,events,error}; the run-output directory layout and cp-native/docno-on-disk boundary are unchanged; run_output rewrites cp->docno for the new judge/revisit/search/list_exhausted event shapes
- [x] #15 Trace stays heavy/detailed: per-query propose, per-page search, per-NEW-doc judge (cp+grade+reason), per-revisit revisit (cp+grade), list_exhausted (with depth K), bounce (engine_error/zero-result), and stop(reason) events, plus llm_call per round-trip; a caught mid-loop LLM failure yields a partial result and is never dropped
- [x] #16 Tests (FakeEngine + stub LLMs, no network) cover: Judger parallel/stub judging returning cp-less 0-3 Verdicts; wave-judging in batches of concurrency with RETAIN-ALL (the tripping wave fully recorded); a large fetch descended client-side with a continuation fetch (exclude=seen) when the streak is not reached; prior-judged docs counted-not-rejudged; streak (grade-0 default) over the true list in rank order; the depth-K / J / X / Y aggregate; the run-total budget split and per-intent intent_budget stop; malformed-query + zero-result self-correction; and a judge failure yielding a partial result
- [ ] #17 isj/README.md documents the Searcher/Judger split, the 0-3 trust-aware Judger, retain-all + grade-0 streak, the de-duplication rule, the run-total budget split, and the serving-scout-derived defaults (concurrency 15, reasoning_effort high, max_doc_chars 50000)
<!-- AC:END -->

## Implementation Plan

<!-- SECTION:PLAN:BEGIN -->
Detailed, reviewed implementation plan (grounded in the existing isj_agent code + the Judger serving scout). Implement as ONE PR on searcher-judger in staged commits. NO C++ changes.
...
Implemented and merged-ready on PR #6 (branch searcher-judger). Delivered the Searcher/Judger split: a query-only Searcher (single search tool, tool_choice=required, reasoning_effort high), a parallel full-document Judger (pointwise UMBRELA 0-3 + trust, cp-less Verdict{reason,grade Literal[0,1,2,3]}, ThreadPoolExecutor at concurrency 15 per the serving scout), and a Controller owning wave-judging, retain-all, the grade-0 non-relevant streak, per-query exclude de-duplication, the run-total max_judgments split across intents, error self-correction, and a heavy verbatim trace. Protocol: Verdict replaces Judgement; RankedEntry.grade -> Literal[0,1,2,3]. Wiring: orchestrator budget split, cli/config ([agents.judger], [loop]), run_output cp->docno for new events. Scout-derived defaults (concurrency 15, reasoning_effort high, max_doc_chars 50000) from isj/scouting/scout_judger.py. Post-implementation refinements landed on the same branch: a thorough GCL-teaching searcher.md (proximity (>> (# N) ...), register diversity, the returned-JSON contract), atom_counts surfaced to the agent (TASK-17), and result fields reordered to rank/score/summary/reason/grade. Validation: 87 unit tests pass (FakeEngine + stub LLMs); a live E2E smoke run and multiple eval runs (croup, Yellowstone) against the real vLLM gpt-oss-120b + cottontail-jsonl-server. Known follow-ups (unfiled): no-progress backstop, entity-anchor pivot strategy, query-length/repetition guard, judge calibration/consistency, register diversity.
<!-- SECTION:FINAL_SUMMARY:END -->

## Definition of Done
<!-- DOD:BEGIN -->
- [x] #1 uv run --project isj pytest tests/ passes
- [x] #2 relevant_grade_threshold defaults to 1 (non-relevant streak = grade 0) and is configurable; judge defaults from the serving scout are wired: [agents.judger].concurrency=15, reasoning_effort='high', max_doc_chars=50000
<!-- DOD:END -->
