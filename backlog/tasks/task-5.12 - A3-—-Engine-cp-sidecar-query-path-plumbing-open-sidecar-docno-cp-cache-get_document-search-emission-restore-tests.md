...
- [ ] #2 get_document accepts a cp OR a docno. By cp: translate(cp, cq) (cq from the :item container at cp), no map. By docno (the CLI --get human case): read the SQLite map <burrow>/docno-cp.sqlite (read-only) for docno->cp, then translate -- a boundary op, the only place C++ touches SQLite; the hot cover_search/exclusion path stays map-free. An unknown docno/cp is not-found (not an error).
...
- [ ] #4 cottontail-jsonl-query --get accepts a docno (reads the SQLite map read-only) or a cp; --text/--gcl emit cp; the server /tools/{get_document,search_text,search_gcl} use the cp-native functions. C++ gains a READ-ONLY SQLite dependency (Bazel) used solely by the boundary get-by-docno; the hot path is map-free.
...
<!-- SECTION:NOTES:BEGIN -->
RE-SPEC cp-native (doc-6, 2026-06-21; refined 2026-06-22 for issue #8). The HOT path is sidecar/map-free: open_burrow returns just the warren; search returns cp; the :docno hopper is dropped. get_document accepts a cp (hot) OR a docno (the CLI --get boundary case, which reads the SQLite map <burrow>/docno-cp.sqlite READ-ONLY -- a read-only SQLite dependency used solely there). cp is on the wire; the cp->docno rewrite for persisted output is C2. Authoritative: doc-6 + docs/indexing.md.
<!-- SECTION:NOTES:END -->
