...
FOLLOW-UP FIX (2026-07-18): the "lazy byte backstop" the earlier notes describe (load_cache evicting idle to keep large_bytes_ under budget for non-reserving paths) was itself the cause of a severe budget breach and is now REMOVED. Field symptom: a 32 GB-budget shard server spiked to ~56 GB RSS with 0 OVER BUDGET bounces on a big MultiText tiered program (rag2026-13 intent-01). Root cause, proven by single-shard A/B (docs/investigations/task47-budget-overshoot/): the backstop called evict_idle_locked(target, {}) with an EMPTY protect set, so it evicted large CacheRecords that the parallel ranking workers (parallel_ranking clones share one SimpleIdx cache; each worker opens its own hopper on every leaf -> use_count 2,3,4) still held. Erasing a pinned entry frees no memory (the hopper keeps the shared_ptr alive) but decrements large_bytes_, so the budget looks like it has room and the same big stop-word list (and/of/to/a, 6-8 GB each) is re-decompressed into a DUPLICATE copy on the next tier/worker. Measured on one shard, budget 32, rank-threads 4: peak VmHWM 97.6 GB with 88 pinned evictions; with the backstop off, 34.7 GB and 0 evictions (identical to the budget-100 control). Fix (commit 39a080d): drop the eviction from load_cache, keep the LRU accounting (large_bytes_, ages_). Eviction now lives ONLY in reserve() (admission control), which protects the needed set and runs between queries -- serialized by the g_ranking_mutex -- when nothing is pinned, so it can never evict a pinned feature. Re-running the exact scenario after the fix: peak VmHWM 39.65 GB (query A + a disjoint query B), 0 duplication, both queries correct; //test:tests //test:hazel_test //test:jsonl_test //test:jsonl_server_test all green. NOTE this supersedes the AC#5/implementation-notes claim that "load_cache keeps a lazy byte backstop for non-reserving paths": there is no backstop now; reserve() admission is the sole memory control, which is correct for the serialized one-materializing-query-at-a-time workload. Residual: a bounded ~7 GB overshoot over B remains (transient compressed-blob decompression scratch + a small admission under-count of un-budgeted small features); ACCEPTED by the operator (Mark) as not worth chasing -- it is a few-GB effect, not the runaway.

PRODUCTION VALIDATION (2026-07-24): the backstop fix (commit 39a080d) is validated end-to-end on the real 8-shard ClimbMix stack under the ISJ Searcher. A full 119-topic gcl run (config-gcl-cover.toml) at budget 36 GB/server, with an RSS guard armed to abort any server at 48 GB, COMPLETED over ~5 days with NO OOM, NO crash, NO guard abort: peak busiest server 42.7 GB (under the 44 GB warn / 48 GB abort) across ~90k samples, peak all-8 total 339 GB. All 119 topics produced output + DONE, no errors.log, no engine timeouts, no tracebacks; ~1540 judged passages/topic median (174,754 total). The admission guard fired exactly 2 OVER BUDGET bounces in the whole run (stop-word-heavy programs naming the=14.8/to=7.5/of=7.5/a=6.4 GB) with the drop-stop-words coaching, and both topics still completed; no other query breached. rag2026-13 -- the topic whose COO program produced the original 56 GB field breach -- completed cleanly on the fixed build. The residual ~few-GB overshoot over B (fix-2: un-budgeted small features + transient decompression scratch) manifested as the 42.7 GB peak vs the 36 GB budget and was ACCEPTED by the operator as out of scope. Full write-up incl. per-arm error sweep in docs/investigations/task47-budget-overshoot/README.md.
<!-- SECTION:NOTES:END -->
