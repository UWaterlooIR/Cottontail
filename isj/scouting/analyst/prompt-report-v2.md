...
- Each component is a **self-contained, retrieval-ready statement** of one distinct
  ...
  component on its own — without the original need — and know what to go find.
...
- Together, the components must **cover the whole need**: every part of what the user
  is asking for should map to at least one component, and nothing important should be
  left out. Missing a component means the report will have a hole.
...
- **Split only when different documents would answer each part.** If a single document
  would naturally cover several facets together — the *pros and cons* of one thing, the
  arguments *for and against* one position, or two closely parallel subjects — keep them
  in ONE component. Split into separate components only when each targets a genuinely
...
COMPONENTS:
1. The main advantages and disadvantages of nuclear energy for electricity generation, including cost, reliability, and radioactive waste.
...
Why these work: "pros" and "cons" are ONE component (1), because a single source on
nuclear energy discusses both — separate pros/cons components would retrieve the same
documents. There is NO broad "overview of nuclear energy" component; it would overlap
everything. Each named, specific subject (Chernobyl, Bison Energy, Peninsula Clean
Energy) is its own component because each is answered by a different set of documents.
Every component names information to find, written as a statement, with no request for
...
- `interpretations`: the ordered list of component strings (most central to the report
  first).
