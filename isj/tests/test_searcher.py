...
def _tool_call(query, cid="c1"):
    return SimpleNamespace(
        ...
        function=SimpleNamespace(name="search", arguments=json.dumps({"query": query})),
    )
...
def test_propose_returns_query_and_appendable_assistant_message():
    ...
    assert r.assistant_message["tool_calls"][0]["function"]["name"] == "search"
...
def test_propose_forces_the_search_tool():
    ...
    assert names == ["search"]  # exactly one tool offered; there is no judge tool
...
def test_no_tool_call_yields_none_query_defensively():
    ...
    r = s.propose([{"role": "user", "content": "q"}])
    assert r.query is None and r.n_tool_calls == 0
