...
def write_flat(path, rows):
    path.write_text("".join(f"{docid}\t{cp}\n" for docid, cp in rows), encoding="utf-8")


def test_build_and_round_trip(tmp_path):
    rows = [("doc-001", 0), ("doc-002", 9), ("doc-003", 16)]
    flat = tmp_path / "docid-cp.tsv"
    ...
    with DocnoMap(sqlite_path) as m:
        for docid, cp in rows:
            assert m.cp(docid) == cp
            assert m.docno(cp) == docid
...
def test_duplicate_docid_named(tmp_path):
    flat = tmp_path / "docid-cp.tsv"
